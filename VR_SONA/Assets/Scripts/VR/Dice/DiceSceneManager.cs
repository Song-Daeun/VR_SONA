...
public class DiceSceneManager : MonoBehaviour
{
    ...
    private void CheckDiceState()
    {
        ...
        // 주사위가 멈췄는지 확인
        bool isStill = diceRigidbody.velocity.magnitude < stoppedVelocityThreshold &&
                       diceRigidbody.angularVelocity.magnitude < stoppedAngularThreshold;
        ...
        if (isStill && isRolling)
        {
            ...
            if (showDebugLogs && stoppedTimer % 0.5f < Time.deltaTime)
            {
                Debug.Log($"주사위 정지 중... ({stoppedTimer:F1}초)");
            }
        ...
        {
            // 주사위가 다시 움직이기 시작
            if (!isRolling && showDebugLogs)
            {
                Debug.Log("주사위가 굴러가기 시작");
            }
            ...
            // 이전 결과 숨기기
            if (resultUI != null && resultUI.resultPanel.activeSelf)
            {
